    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->banner_img && $model->validate()) {
                //banner img
                $fileName = $model->generateRandomString(10) . $model->banner_img->baseName;
                $ext = $model->banner_img->extension;
                ...
                $model->banner_img->saveAs($targetPath);
                $model->banner_img = $fileName . '.' . $ext;
                $model->save(false);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if (UploadedFile::getInstance($model, 'banner_img')) {
                ...
                if ($model->validate()) {
                    ...
                    $url = Yii::getAlias('@webroot') . '/../../uploads/slider/' . $img;
                    @unlink($url);
                    $fileName = $model->generateRandomString(10) . $model->banner_img->baseName;
                    $ext = $model->banner_img->extension;
                    $path = Yii::getAlias('@webroot') . '/../../uploads/slider/';
